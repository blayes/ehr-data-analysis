...
if (mtd == 1) {
    ...
    fit.ksvp.sp <- ksvm(as.character(xtrain.icd[ , 2]), as.factor(ytrain), kernel = sks, scale = c(), prob.model = TRUE)
    fit.ksvp.bd <- ksvm(as.character(xtrain.icd[ , 2]), as.factor(ytrain), kernel = skb, scale = c(), prob.model = TRUE)

    fit.gp.sp <- gausspr(as.character(xtrain.icd[ , 2]), as.factor(ytrain), kernel = sks, scale = c(), prob.model = TRUE)
    fit.gp.bd <- gausspr(as.character(xtrain.icd[ , 2]), as.factor(ytrain), kernel = skb, scale = c(), prob.model = TRUE)

    pred.ksvp.sp <- predict(fit.ksvp.sp, as.character(xtest.icd[ , 2]), type = "probabilities")[ , 2]
    pred.ksvp.bd <- predict(fit.ksvp.bd, as.character(xtest.icd[ , 2]), type = "probabilities")[ , 2]

    pred.gp.sp <- predict(fit.gp.sp, as.character(xtest.icd[ , 2]), type = "probabilities")[ , 2]
    pred.gp.bd <- predict(fit.gp.bd, as.character(xtest.icd[ , 2]), type = "probabilities")[ , 2]
    ...
    fname <- paste0("../data/comp_sim2_", id, ".rds")
    saveRDS(res, fname)
} else if (mtd == 2) {
    ...
    fname <- paste0("../data/gp_sim2_", id, ".rds")
    ...
    dat <- readRDS("../data/sim_data_nonlin_corr_reg.rds")
    cvdat <- dat[[id]]
    ...
    fit.ksvp.sp <- ksvm(as.character(xtrain.icd[ , 2]), ytrain, kernel = sks, scale = c())
    fit.ksvp.bd <- ksvm(as.character(xtrain.icd[ , 2]), ytrain, kernel = skb, scale = c())

    fit.gp.sp <- gausspr(as.character(xtrain.icd[ , 2]), ytrain, kernel = sks, scale = c(), variance.model = TRUE)
    fit.gp.bd <- gausspr(as.character(xtrain.icd[ , 2]), ytrain, kernel = skb, scale = c(), variance.model = TRUE)

    pred.ksvp.sp <- predict(fit.ksvp.sp, as.character(xtest.icd[ , 2]), type = "response")
    pred.ksvp.bd <- predict(fit.ksvp.bd, as.character(xtest.icd[ , 2]), type = "response")

    pred.gp.sp <- predict(fit.gp.sp, as.character(xtest.icd[ , 2]), type = "response")
    pred.gp.bd <- predict(fit.gp.bd, as.character(xtest.icd[ , 2]), type = "response")
    ...
    fname <- paste0("../data/comp_sim2_reg_", id, ".rds")
    saveRDS(res, fname)
} else if (mtd == 4) {
    estimateCodeFreq <- function(dx) {
        unq_codes = as.character(dx)
    ...
    fname <- paste0("../data/gp_sim2_reg_", id, ".rds")
